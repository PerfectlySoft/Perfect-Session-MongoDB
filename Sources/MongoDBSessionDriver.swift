...
public struct SessionMongoDBDriver {
	...
	public init() {
		let filter = SessionMongoDBFilter()
		requestFilter = (filter, HTTPFilterPriority.high)
		responseFilter = (filter, HTTPFilterPriority.high)
	}
}
public class SessionMongoDBFilter {
	var driver = MongoDBSessions()
	public init() {
		_ = PerfectSessionClass(runSetup: true) // runs db setup
	}
}

extension SessionMongoDBFilter: HTTPRequestFilter {
...
extension SessionMongoDBFilter: HTTPResponseFilter {
